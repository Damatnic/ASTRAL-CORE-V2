...
function Set-VercelEnvVar {
    ...
    vercel env add $Name $Value --scope $Target
}
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "2. Set up a production database"
    Write-Host "3. Configure external services as needed"
    Write-Host "4. Test the deployment thoroughly"
} else {
    Write-Host ""
    Write-Host "Deployment failed. Please check the output above." -ForegroundColor Red
}