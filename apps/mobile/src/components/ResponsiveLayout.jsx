...
import { View, ScrollView, SafeAreaView, StyleSheet, Dimensions, Platform, KeyboardAvoidingView, StatusBar, } from 'react-native';
...
export const useResponsive = () => {
    ...
    const [deviceType, setDeviceType] = useState(getDeviceType(SCREEN_WIDTH));
    const [orientation, setOrientation] = useState(SCREEN_WIDTH < SCREEN_HEIGHT ? 'portrait' : 'landscape');
    useEffect(() => {
...
export const ResponsiveLayout = ({ children, scrollable = true, keyboardAware = true, padding = true, backgroundColor = '#FFFFFF', safeArea = true, footer, header, }) => {
    ...
    const content = (<MainContainer style={containerStyle}>
      {header && <View style={styles.header}>{header}</View>}
      
      <ContentWrapper style={scrollable ? undefined : contentStyle} contentContainerStyle={scrollable ? styles.scrollContent : undefined} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        {scrollable ? (<View style={contentStyle}>{children}</View>) : (children)}
      </ContentWrapper>

      {footer && (<View style={[styles.footer, { paddingBottom: insets.bottom }]}>
          {footer}
        </View>)}
    </MainContainer>);
    if (keyboardAware && Platform.OS === 'ios') {
        return (<KeyboardAvoidingView style={{ flex: 1 }} behavior="padding" keyboardVerticalOffset={0}>
        {content}
      </KeyboardAvoidingView>);
    }
    return content;
};
function getResponsiveStyles(deviceType, orientation, dimensions) {
...
export const ResponsiveGrid = ({ children, columns, gap = 12, }) => {
    ...
    return (<View style={{ marginHorizontal: -gap / 2 }}>
      {Array.from({ length: rows }).map((_, rowIndex) => (<View key={rowIndex} style={{
                flexDirection: 'row',
          ...
          {Array.from({ length: cols }).map((_, colIndex) => {
                ...
                if (index >= children.length)
                    return null;
                return (<View key={colIndex} style={{
                        flex: 1,
...
// Responsive Spacing
export const getResponsiveSpacing = (baseSpacing, deviceType) => {
    switch (deviceType) {
        case 'phone':
            return baseSpacing;
        case 'tablet':
...
//# sourceMappingURL=ResponsiveLayout.jsx.map