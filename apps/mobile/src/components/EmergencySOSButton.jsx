...
import { View, Text, StyleSheet, Animated, Alert, Vibration, Platform, Dimensions, Pressable, } from 'react-native';
import * as Haptics from 'expo-haptics';
import * as Location from 'expo-location';
import { Ionicons } from '@expo/vector-icons';
import AsyncStorage from '@react-native-async-storage/async-storage';
const { width: SCREEN_WIDTH } = Dimensions.get('window');
const EmergencySOSButton = ({ onEmergencyActivated, style, size = 'large', floating = false, }) => {
    ...
    const activateEmergency = async () => {
        ...
        try {
            ...
            // Show confirmation
            Alert.alert('Emergency Activated', 'Help is on the way. A crisis counselor will contact you immediately.', [
                {
            ...
            Alert.alert('Emergency Alert Sent', 'We couldn\'t get your location, but help is on the way.', [{ text: 'OK' }]);
        }
    };
    const cancelEmergency = async () => {
        ...
        Alert.alert('Emergency Cancelled', 'The emergency request has been cancelled.', [{ text: 'OK' }]);
    };
    const buttonStyle = [
        styles.button,
    ...
    return (<Animated.View style={[
      ...
      <Pressable onPressIn={handlePressIn} onPressOut={handlePressOut} style={({ pressed }) => [
        ...
        <View style={styles.buttonContent}>
          <Ionicons name="warning" size={sizeConfig.iconSize} color="#FFFFFF"/>
          <Text style={[styles.buttonText, { fontSize: sizeConfig.fontSize }]}>
            {isActivated ? 'ACTIVE' : countdown !== null ? countdown.toString() : 'SOS'}
          </Text>
          {!isActivated && size === 'large' && (<Text style={styles.instructionText}>Hold 3 seconds</Text>)}
        </View>
      </Pressable>

      {/* Visual feedback rings */}
      {isPressed && !isActivated && (<View style={[styles.ripple, { width: sizeConfig.width * 1.5, height: sizeConfig.width * 1.5 }]}/>)}
      {isActivated && (<View style={[styles.activeRing, { width: sizeConfig.width * 1.3, height: sizeConfig.width * 1.3 }]}/>)}
    </Animated.View>);
};
...
//# sourceMappingURL=EmergencySOSButton.jsx.map