...
class PushNotificationService {
    ...
    async sendEmergencyNotification(message, location) {
        await this.sendLocalNotification({
            type: 'emergency',
    ...
    async scheduleCheckInNotifications() {
    ...
    async scheduleCrisisFollowUp(delayMinutes = 30) {
        await Notifications.scheduleNotificationAsync({
            content: {
    ...
    handleCrisisAlert(notification) {
        console.log('Crisis alert received:', notification);
    }
    handleCheckInReminder(notification) {
