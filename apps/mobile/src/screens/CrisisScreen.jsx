...
export default function CrisisScreen() {
    ...
    const [criticalTips, setCriticalTips] = useState([]);
    useEffect(() => {
        loadOfflineData();
    }, []);
    ...
    const handleStartCrisisSession = async () => {
        try {
            Alert.alert('Crisis Session', 'How severe is your current situation?', [
                {
                    text: 'Mild',
    ...
    const startSession = async (severity) => {
        try {
            setIsInCrisis(true);
            const session = await CrisisService.startCrisisSession(severity);
            // Send emergency notification
            await NotificationService.getInstance().sendEmergencyAlert(`Crisis session started with ${severity} severity`);
            if (severity === 'critical') {
                Alert.alert('Emergency Services', 'For life-threatening situations, please call emergency services immediately.', [
                    {
                        text: 'Call 911',
    ...
    const handleEndCrisisSession = () => {
        Alert.alert('End Crisis Session', 'Are you feeling safer now?', [
            {
    ...
    const renderEmergencyContacts = () => (<View style={styles.section}>
      <Text style={styles.sectionTitle}>🚨 Emergency Contacts</Text>
      {emergencyContacts.map((contact) => (<TouchableOpacity key={contact.id} style={[
    ...
    const renderCriticalTips = () => (<View style={styles.section}>
      <Text style={styles.sectionTitle}>💡 Immediate Help</Text>
      {criticalTips.map((tip) => (<View key={tip.id} style={styles.tipCard}>
    ...
    return (<ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
      {!isInCrisis ? (<>
          <View style={styles.heroSection}>
            ...
            <TouchableOpacity style={styles.crisisButton} onPress={handleStartCrisisSession}>
          ...
          <TouchableOpacity style={styles.escalateButton} onPress={() => CrisisService.escalateToEmergency()}>
            <Text style={styles.escalateButtonText}>🚨 Escalate to Emergency</Text>
          </TouchableOpacity>

          <TouchableOpacity style={styles.endSessionButton} onPress={handleEndCrisisSession}>
...
//# sourceMappingURL=CrisisScreen.jsx.map