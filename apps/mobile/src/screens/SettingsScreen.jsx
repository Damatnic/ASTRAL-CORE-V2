...
export default function SettingsScreen() {
    ...
    const handleBiometricToggle = async (enabled) => {
        ...
        if (enabled) {
            Alert.alert('Enable Biometric Lock', 'This will require biometric authentication to access the app', [
    ...
    const handleEmergencyContactsSetup = () => {
        Alert.alert('Emergency Contacts', 'Set up emergency contacts for crisis situations', [
            { text: 'Cancel', style: 'cancel' },
            { text: 'Setup', onPress: () => console.log('Navigate to emergency contacts setup') },
        ]);
    };
    const handleDataExport = () => {
        Alert.alert('Export Data', 'Export your crisis session data and preferences', [
            { text: 'Cancel', style: 'cancel' },
    ...
    const handleDeleteAccount = () => {
        Alert.alert('Delete Account', 'This will permanently delete your account and all associated data. This action cannot be undone.', [
            { text: 'Cancel', style: 'cancel' },
            {
                ...
                style: 'destructive',
                onPress: () => {
                    Alert.alert('Final Confirmation', 'Are you absolutely sure you want to delete your account?', [
                        { text: 'Cancel', style: 'cancel' },
    ...
    const handleContactSupport = () => {
        Alert.alert('Contact Support', 'Get help with technical issues or crisis support questions', [
            { text: 'Cancel', style: 'cancel' },
    ...
    const renderSettingItem = (item) => {
        ...
        return (<View key={item.id} style={itemStyle}>
        ...
        <View style={styles.settingControl}>
          {item.type === 'toggle' && (<Switch value={item.value || false} onValueChange={item.onToggle} disabled={item.disabled} trackColor={{ false: '#D1D5DB', true: '#3B82F6' }} thumbColor={item.value ? '#FFFFFF' : '#FFFFFF'}/>)}
          
          {(item.type === 'button' || item.type === 'navigation') && (<TouchableOpacity style={[
                    styles.actionButton,
                    item.danger && styles.dangerButton,
                    item.disabled && styles.disabledButton,
                ]} onPress={item.onPress} disabled={item.disabled}>
              <Text style={[
          ...
          {item.type === 'info' && (<Text style={styles.infoText}>v2.0.0</Text>)}
        </View>
      </View>);
    };
    const renderSection = (section) => (<View key={section.title} style={styles.section}>
