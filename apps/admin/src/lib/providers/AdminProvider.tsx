...
interface AdminContextType {
  // Direct access to data properties (for component convenience)
  alerts: Alert[];
...
export function AdminProvider({ children }: AdminProviderProps) {
  ...
  const mapVolunteerStatus = (status: string): 'online' | 'offline' | 'busy' | 'away' => {
    switch (status) {
      case 'available':
        return 'online';
      case 'emergency':
  ...
  const mapConnectionStatus = (status: string): 'connected' | 'disconnected' | 'error' => {
    switch (status) {
      case 'connected':
        return 'connected';
      case 'connecting':
      ...
      if (response.ok) {
        setStore(prev => ({
          ...
          alerts: prev.alerts.map(alert =>
            alert.id === alertId
              ? { ...alert, priority: 'critical' as PriorityLevel }
              : alert
          )
        }));
      }
  ...
  const assignVolunteerToSession = async (sessionId: string, volunteerId: string) => {
    try {
  ...
  const transformedVolunteers = store.volunteers.map((volunteer: any) => ({
    id: volunteer.id,
    name: volunteer.name,
