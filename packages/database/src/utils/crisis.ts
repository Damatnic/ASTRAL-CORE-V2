...
import { prisma } from '../';
import { MessageSender, EscalationType, EscalationTrigger } from '../generated/client';
 ...
 */
export async function createCrisisSession(params: CreateCrisisSessionParams) {
 ...
 */
export async function findUnassignedCrisisSessions(limit: number = 10) {
...
export async function storeCrisisMessage(
  ...
  return await prisma.crisisMessage.create({
    data: {
      sessionId,
      senderType: senderType as any, // MessageSender enum
      senderId,
      encryptedContent,
      messageHash,
      sentimentScore: metadata?.sentimentScore,
      riskScore: metadata?.riskScore,
      keywordsDetected: metadata?.keywordsDetected ? JSON.stringify(metadata.keywordsDetected) : null,
    },
  });
}
...
export async function escalateToEmergency(
  ...
  const sessionUpdate = prisma.crisisSession.update({
    ...
    data: {
      ...
      escalationType: escalationType as any, // EscalationType enum
    },
  });
  ...
  const escalationRecord = prisma.crisisEscalation.create({
    data: {
      sessionId,
      triggeredBy: trigger as any, // EscalationTrigger enum
      severity: 'EMERGENCY',
      reason,
      actionsTaken: JSON.stringify(actionsTaken), // Convert array to string
 ...
 */
export async function getCrisisSessionStats(timeframe: 'hour' | 'day' | 'week' = 'hour') {
 ...
 */
export async function cleanupExpiredSessions(maxAgeHours: number = 24) {
