 ...
 */
export function deriveUserKey(userId, masterSecret) {
 ...
 */
export function encryptUserData(data, userId) {
    try {
 ...
 */
export function decryptUserData(encryptedData, userId, salt, iv, tag) {
    try {
        const key = deriveUserKey(userId);
        const crypto = require('crypto');
 ...
 */
export function verifyContentHash(data, hash) {
    return createContentHash(data) === hash;
}
/**
 * Encrypts mood entry data
 */
export function encryptMoodEntry(moodData, userId) {
 ...
 */
export function decryptMoodEntry(encryptedData, userId, metadata) {
    const decryptedJson = decryptUserData(encryptedData, userId, metadata.salt, metadata.iv, metadata.tag);
 ...
 */
export function encryptSafetyPlan(planData, userId) {
 ...
 */
export function decryptSafetyPlan(encryptedContent, userId, contentHash, salt, iv, tag) {
 ...
 */
export function encryptUserProfile(profileData, userId) {
 ...
 */
export function decryptUserProfile(encryptedProfile, userId, metadata) {
    const decryptedJson = decryptUserData(encryptedProfile, userId, metadata.salt, metadata.iv, metadata.tag);
    // Verify data integrity
    if (!verifyContentHash(decryptedJson, metadata.hash)) {
