...
import { prisma } from '../';
import { SafetyPlan, SafetyPlanVersion, EmergencyNotificationType } from '../generated/client';
...
export interface SafetyPlanData {
  warningSignsInternal: string[];      // Internal warning signs
  warningSignsExternal: string[];      // External triggers
  copingStrategies: string[];          // Self-soothing activities
  socialSupports: string[];            // People to contact
  professionalContacts: string[];      // Mental health professionals
  environmentalSafety: string[];       // Making environment safer
  emergencyContacts: string[];         // Emergency contacts
  reasonsToLive: string[];            // Reasons for living
  crisisNumbers: string[];            // Crisis helpline numbers
  
  // Advanced features
  riskFactors: string[];              // Personal risk factors
  protectiveFactors: string[];        // Personal protective factors
  medications: string[];              // Current medications
  medicalConditions: string[];        // Relevant medical conditions
  
  // Personalization
  preferredCopingMethods: string[];   // Most effective coping methods
  pastEffectiveStrategies: string[];  // What worked before
  triggerPatterns: string[];          // Pattern recognition
 ...
 */
export async function createSafetyPlan(params: CreateSafetyPlanParams): Promise<SafetyPlan> {
 ...
 */
export async function updateSafetyPlan(params: UpdateSafetyPlanParams): Promise<SafetyPlan> {
  const start = Date.now();
 ...
 */
export async function getSafetyPlanData(planId: string, userId: string): Promise<SafetyPlanData> {
  try {
    ...
    const decryptedData = decryptSafetyPlan(
      Buffer.from(plan.encryptedContent),
      userId,
      plan.contentHash,
      Buffer.alloc(32), // Salt - would be stored separately in production
 ...
 */
export async function getUserSafetyPlans(userId: string): Promise<SafetyPlan[]> {
 ...
 */
export async function activateSafetyPlan(
  planId: string,
  userId: string,
  sessionId?: string,
  ...
  try {
    ...
    if (plan.emergencyShare && planData.shareWithEmergencyContacts) {
      ...
      await triggerEmergencyNotifications(
        userId,
 ...
 */
export async function getSafetyPlanVersions(planId: string, userId: string): Promise<SafetyPlanVersion[]> {
 ...
 */
export async function shareSafetyPlan(
  planId: string,
  userId: string,
  shareWithEmergencyContacts: boolean,
  allowProfessionalAccess: boolean
 ...
 */
export async function createSafetyPlanFromTemplate(
  userId: string,
  templateType: 'suicide_prevention' | 'anxiety_management' | 'depression_support' | 'general_wellness'
): Promise<SafetyPlan> {
  const templates = {
    suicide_prevention: {
 ...
 */
export async function checkSafetyPlanHealth(): Promise<{
