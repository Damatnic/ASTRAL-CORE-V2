 ...
 */
export function encryptContactData(name, phone, userId, email) {
 ...
 */
export function decryptContactData(contact, userId) {
    try {
        ...
        const name = decryptUserData(Buffer.from(contact.encryptedName), userId, Buffer.from(contact.keyDerivationSalt), Buffer.alloc(16), // IV - would be stored separately in production
        Buffer.alloc(16) // Auth tag - would be stored separately in production
        );
        const phone = decryptUserData(Buffer.from(contact.encryptedPhone), userId, Buffer.from(contact.keyDerivationSalt), Buffer.alloc(16), Buffer.alloc(16));
        let email;
        if (contact.encryptedEmail) {
            email = decryptUserData(Buffer.from(contact.encryptedEmail), userId, Buffer.from(contact.keyDerivationSalt), Buffer.alloc(16), Buffer.alloc(16));
        }
...
export async function createEmergencyContact(params) {
    ...
    try {
        ...
        const encryptedData = encryptContactData(params.name, params.phone, params.userId, params.email);
 ...
 */
export async function getUserEmergencyContacts(userId) {
 ...
 */
export async function getAutoNotifyContacts(userId) {
    return await prisma.emergencyContact.findMany({
        where: {
            userId,
            isActive: true,
 ...
 */
export async function notifyEmergencyContact(params) {
    const start = Date.now();
    try {
 ...
 */
export async function triggerEmergencyNotifications(userId, notificationType, severity, message, sessionId, tetherEmergencyId) {
    const start = Date.now();
    try {
        ...
        const notifications = await Promise.all(contacts.map(contact => notifyEmergencyContact({
 ...
 */
export async function updateNotificationStatus(notificationId, status, deliveredAt, acknowledgedAt, responseReceived, errorMessage) {
    const updateData = {
        status,
        deliveredAt,
        acknowledgedAt,
 ...
 */
export async function verifyEmergencyContact(contactId) {
 ...
 */
export async function updateContactConsent(contactId, hasConsent, consentDate) {
    return await prisma.emergencyContact.update({
        where: { id: contactId },
        data: {
            hasConsent,
 ...
 */
export async function getEmergencyNotificationHistory(userId, timeframe = 'week') {
 ...
 */
export async function checkEmergencyContactHealth() {
    const start = Date.now();
    try {
        const since24h = new Date(Date.now() - 24 * 60 * 60 * 1000);
        const [totalContacts, verifiedContacts, consentedContacts, recentNotifications, avgResponseTime,] = await Promise.all([
