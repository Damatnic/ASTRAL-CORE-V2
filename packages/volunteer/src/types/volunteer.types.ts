...
export interface VolunteerAvailability {
  ...
  minimumBreakBetweenSessions: number; // in minutes
...
export interface SessionPreferences {
  ...
  preferShortSessions: boolean; // under 30 minutes
  preferLongSessions: boolean; // over 60 minutes
...
export interface WellnessMetrics {
  currentBurnoutScore: number; // 0-100, higher = more burnout risk
...
export interface PerformanceMetrics {
  ...
  sessionQualityScore: number; // 1-10 scale
  responseTimeAverage: number; // in seconds
  crisisResolutionRate: number; // percentage
  clientSatisfactionScore: number; // 1-10 scale
  professionalDevelopmentScore: number; // 1-10 scale
...
export interface VolunteerTeam {
  ...
  name: string;
  description: string;
  teamLead: string; // volunteer ID
  members: string[]; // volunteer IDs
...
export interface PerformanceDistribution {
  excellent: number; // 9-10 rating
  good: number; // 7-8 rating
  satisfactory: number; // 5-6 rating
  needsImprovement: number; // 1-4 rating
}
...
export interface TrainingCompletionRate {
  ...
  averageScore: number;
  timeTaken: number; // average days to complete
}