...
export class VolunteerTrainingSystem {
     ...
     */
    async startTraining(volunteerId, moduleId) {
        try {
     ...
     */
    async completeTraining(volunteerId, moduleId, score, timeSpent) {
        try {
            const module = this.trainingModules.get(moduleId);
            if (!module) {
                return { success: false, message: 'Training module not found' };
     ...
     */
    async checkPrerequisites(volunteerId, prerequisites) {
     ...
     */
    async checkCertificationEligibility(volunteerId) {
        const volunteerProgress = this.volunteerProgress.get(volunteerId) || [];
        ...
        for (const [level, cert] of this.certifications.entries()) {
            const hasAllRequired = cert.requiredModules.every(module => completedModules.includes(module));
            if (hasAllRequired) {
                return true;
            }
        }
     ...
     */
    async awardCertification(volunteerId) {
        try {
            const volunteerProgress = this.volunteerProgress.get(volunteerId) || [];
            const completedModules = volunteerProgress
                .filter(p => p.status === 'completed')
     ...
     */
    getCertificationLevel(volunteerId) {
        const volunteerProgress = this.volunteerProgress.get(volunteerId) || [];
        const completedModules = volunteerProgress
            .filter(p => p.status === 'completed')
            .map(p => p.moduleId);
...
//# sourceMappingURL=VolunteerTrainingSystem.js.map