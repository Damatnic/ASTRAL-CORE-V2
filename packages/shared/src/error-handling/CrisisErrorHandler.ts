...
class CrisisErrorHandler {
  private static instance: CrisisErrorHandler;
  
  private readonly config: ErrorHandlingConfig;
  private readonly circuitBreakers = new Map<string, CircuitBreakerState>();
  private readonly errorStats = new Map<string, { count: number; lastOccurrence: Date }>();
  ...
  public static getInstance(config?: Partial<ErrorHandlingConfig>): CrisisErrorHandler {
  ...
  public createCrisisError(
    ...
    const crisisError = new Error(message) as CrisisError;
  ...
  public async handleError<T>(
    ...
    if (this.isCircuitOpen(operationName)) {
      throw this.createCrisisError(
        `Circuit breaker is open for ${operationName}`,
        ErrorSeverity.HIGH,
        category,
        undefined,
        { circuitBreakerState: this.circuitBreakers.get(operationName) },
        sessionId,
        userId
      );
    }
    ...
    while (attempt < options.maxAttempts!) {
      try {
        ...
        logger.warn(
          'CrisisErrorHandler',
          `Operation ${operationName} failed, retrying in ${delay}ms (attempt ${attempt}/${options.maxAttempts})`,
    ...
    const crisisError = this.createCrisisError(
      `Operation ${operationName} failed after ${attempt} attempts`,
      severity,
      category,
      lastError!,
      { totalAttempts: attempt, ...context },
      sessionId,
      userId
    );
    ...
    if (severity >= ErrorSeverity.CRITICAL) {
      logger.critical(
        'CrisisErrorHandler',
        `CRITICAL FAILURE: ${operationName}`,
        crisisError,
        context,
        sessionId
      );
      ...
      logger.error(
        'CrisisErrorHandler',
        `Operation failure: ${operationName}`,
        crisisError,
        context,
        sessionId
      );
  ...
  public async withTimeout<T>(
    ...
    context: { operationName: string; sessionId?: string } = { operationName: 'unknown' }
  ): Promise<T> {
    return new Promise(async (resolve, reject) => {
      const timeoutId = setTimeout(() => {
        const timeoutError = this.createCrisisError(
          `Operation ${context.operationName} timed out after ${timeoutMs}ms`,
          ErrorSeverity.HIGH,
          ErrorCategory.CRISIS_ENGINE,
          undefined,
          { timeoutMs },
          context.sessionId
        );
        ...
        if (fallback) {
          logger.warn(
            'CrisisErrorHandler',
            `Operation ${context.operationName} timed out, using fallback`,
            { timeoutMs },
            context.sessionId
          );
  ...
  private recordFailure(operationName: string): void {
    ...
    if (breaker.failureCount >= this.config.circuitBreakerThreshold) {
      ...
      logger.warn(
        ...
        { failureCount: breaker.failureCount, threshold: this.config.circuitBreakerThreshold }
      );
    }
  }
  ...
  private getDefaultRetryOptions(): Required<RetryOptions> {
    return {
  ...
  private updateErrorStats(operationName: string): void {
  ...
  private async triggerEmergencyEscalation(error: CrisisError): Promise<void> {
    try {
      ...
      logger.critical(
        'CrisisErrorHandler',
        'EMERGENCY ESCALATION FAILED - SYSTEM IN CRITICAL STATE',
        escalationError as Error,
   ...
   */
  public getErrorStats(): Map<string, { count: number; lastOccurrence: Date }> {
...
// Type guards
export function isCrisisError(error: any): error is CrisisError {
