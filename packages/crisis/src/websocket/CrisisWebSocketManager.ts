...
import type { WebSocketMessage } from '../types/crisis.types';
...
export class CrisisWebSocketManager {
   ...
   */
  async createConnection(config: WebSocketConnectionConfig): Promise<WebSocketConnectionResult> {
  ...
  private generateWebSocketUrl(config: WebSocketConnectionConfig): string {
  ...
  private findSessionByToken(sessionToken: string): string | null {
    for (const connection of this.connections.values()) {
