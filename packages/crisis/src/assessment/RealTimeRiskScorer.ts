...
export interface RiskScoreBreakdown {
  ...
  /** Component scores */
  components: {
    baseRisk: number;        // Keyword and immediate threat analysis
    sentimentRisk: number;   // Emotional state assessment
    behavioralRisk: number;  // Communication patterns
    progressionRisk: number; // Risk escalation over time
  };
...
export class RealTimeRiskScorer {
  ...
  private readonly sessionProgression = new Map<string, RiskProgressionData>();
  ...
  async calculateRiskScore(
    ...
    const riskScore: RiskScoreBreakdown = {
  ...
  private async calculateBaseRisk(assessment: CrisisAssessment): Promise<number> {
  ...
  private async calculateSentimentRisk(assessment: CrisisAssessment, contextData: any): Promise<number> {
    ...
    if (contextData.cognitiveDistortions) {
      const distortionScore = (
        contextData.cognitiveDistortions.hopelessness * 0.4 +
  ...
  private async calculateBehavioralRisk(contextData: any): Promise<number> {
    let behavioralScore = 3; // Baseline behavioral risk
    
    // High-risk behavioral indicators
    if (contextData.hasPlanningLanguage) behavioralScore += 3;
    if (contextData.hasFinalityLanguage) behavioralScore += 4;
    if (contextData.hasIsolationIndicators) behavioralScore += 2;
    if (contextData.hasIncompleteThoughts) behavioralScore += 1;
    if (contextData.hasRapidThoughts) behavioralScore += 1.5;
    ...
    if (contextData.hasFutureWords) behavioralScore -= 1;
    if (contextData.hasSupportWords) behavioralScore -= 0.5;
    if (contextData.cognitiveDistortions?.hope > 0) behavioralScore -= 0.8;
    
    return Math.max(1, Math.min(behavioralScore, 10));
  }
  
  private async calculateProgressionRisk(currentScore: number, sessionId: string): Promise<number> {
  ...
  private calculateWeightedScore(components: {
    ...
    const weightedSum = (
      components.baseRisk * this.COMPONENT_WEIGHTS.BASE_RISK +
  ...
  private determineRiskLevel(score: number): RiskScoreBreakdown['riskLevel'] {
    if (score >= this.RISK_THRESHOLDS.EMERGENCY) return 'EMERGENCY';
  ...
  private determineInterventionLevel(score: number, assessment: CrisisAssessment): RiskScoreBreakdown['interventionLevel'] {
    if (score >= 9 || assessment.emergencyKeywords.length > 0) return 'EMERGENCY';
    if (score >= 7) return 'CRISIS_TEAM';
    if (score >= 5) return 'PROFESSIONAL';
    if (score >= 3) return 'PEER_SUPPORT';
    return 'SELF_HELP';
  }
  
  private requiresImmediateAction(score: number, assessment: CrisisAssessment): boolean {
    return score >= 8 || assessment.emergencyKeywords.length > 0 || assessment.immediateRisk;
  }
  
  private identifyRiskFactors(assessment: CrisisAssessment, contextData: any): string[] {
    ...
    if (contextData.hasPlanningLanguage) factors.push('Planning language detected');
    if (contextData.hasFinalityLanguage) factors.push('Finality language detected');
    if (contextData.hasIsolationIndicators) factors.push('Social isolation indicators');
    if (contextData.hasImmediateTimeWords) factors.push('Immediate time references');
    if (contextData.emotionalIntensity > 7) factors.push('High emotional intensity');
    if (contextData.hasRapidThoughts) factors.push('Rapid thought patterns');
    if (contextData.cognitiveDistortions?.hopelessness > 2) factors.push('Hopelessness expressions');
    if (assessment.sentimentScore < -0.5) factors.push('Very negative sentiment');
    
    return factors;
  }
  
  private identifyProtectiveFactors(assessment: CrisisAssessment, contextData: any): string[] {
    const factors: string[] = [];
    
    if (contextData.hasSupportWords) factors.push('Support system mentioned');
    if (contextData.hasFutureWords) factors.push('Future-oriented thinking');
    if (contextData.cognitiveDistortions?.hope > 0) factors.push('Hope expressions');
    if (assessment.sentimentScore > 0.2) factors.push('Positive sentiment detected');
    if (contextData.copingMatches?.length > 0) factors.push('Coping strategies mentioned');
    
    return factors;
  }
  
  private calculateAssessmentConfidence(assessment: CrisisAssessment, contextData: any): number {
  ...
  private updateSessionProgression(sessionId: string, newScore: number): void {
  ...
  private calculateEscalationTrend(scores: number[]): RiskProgressionData['escalationTrend'] {
    if (scores.length < 3) return 'STABLE';
    
    const trend = this.calculateTrend(scores);
    const recentChange = scores[scores.length - 1] - scores[scores.length - 2];
    
    if (trend > 1.5 || recentChange > 2) return 'RAPIDLY_ESCALATING';
