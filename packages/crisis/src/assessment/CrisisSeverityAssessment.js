 ...
 */
import { RealTimeRiskScorer } from './RealTimeRiskScorer';
export class CrisisSeverityAssessment {
     ...
     */
    async assessMessageWithRiskScore(message, sessionId) {
        ...
        const riskBreakdown = await this.riskScorer.calculateRiskScore(basicAssessment, contextAnalysis, sessionId);
     ...
     */
    calculateRiskEscalationScore(factors) {
        let score = 0;
        // High-risk factors
        if (factors.hasFinalityLanguage)
            score += 4;
        if (factors.hasPlanningLanguage)
            score += 3;
        if (factors.hasIsolationIndicators)
            score += 2;
        // Emotional state factors
        score += Math.min(factors.emotionalIntensity * 0.3, 2);
        // Communication pattern factors
        if (factors.hasIncompleteThoughts)
            score += 1;
    ...
    applyContextualAdjustments(severity, context, message) {
        ...
        if (context.repeatedWords && context.repeatedWords.length > 0) {
            const maxRepeats = Math.max(...context.repeatedWords.map((rw) => rw.count));
            if (maxRepeats > 3) {
