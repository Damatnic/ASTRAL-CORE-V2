...
export class ZeroKnowledgeEncryption {
    ...
    constructor() {
        ...
        if (!this.masterSeed || this.masterSeed.length < 64) {
            throw new Error('CRITICAL SECURITY ERROR: CRISIS_ENCRYPTION_KEY environment variable must be at least 64 characters. ' +
     ...
     */
    generateSessionKeys(sessionToken) {
        ...
        const encryptionKey = crypto.pbkdf2Sync(keyMaterial, salt, CRISIS_CONSTANTS.KEY_DERIVATION_ITERATIONS, 32, // 256 bits for AES-256
        'sha512');
        const sessionKeys = {
            encryptionKey,
            salt,
     ...
     * TARGET: <5ms execution time
     */
    encrypt(plaintext, sessionToken) {
        const start = performance.now();
        ...
        try {
            ...
            const cipher = crypto.createCipheriv(CRISIS_CONSTANTS.ENCRYPTION_ALGORITHM, sessionKeys.encryptionKey, iv);
     ...
     * TARGET: <5ms execution time
     */
    decrypt(encryptedData, sessionToken) {
        const start = performance.now();
        try {
            ...
            if (!sessionKeys && data.salt) {
                ...
                const keyMaterial = sessionToken + this.masterSeed;
                const encryptionKey = crypto.pbkdf2Sync(keyMaterial, salt, CRISIS_CONSTANTS.KEY_DERIVATION_ITERATIONS, 32, 'sha512');
                sessionKeys = {
                    encryptionKey,
                    salt,
                    sessionToken,
            ...
            sessionKeys.lastUsed = new Date();
            // Decrypt message
            const decipher = crypto.createDecipheriv(CRISIS_CONSTANTS.ENCRYPTION_ALGORITHM, sessionKeys.encryptionKey, Buffer.from(data.iv, 'hex'));
     ...
     */
    destroySessionKeys(sessionToken) {
...
//# sourceMappingURL=ZeroKnowledgeEncryption.js.map