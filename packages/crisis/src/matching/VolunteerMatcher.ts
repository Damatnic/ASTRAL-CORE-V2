...
export class VolunteerMatcher {
  private static instance: VolunteerMatcher;
  ...
  async findBestMatch(
    ...
    options: VolunteerMatchingOptions = {}
   ...
   */
  async queueSession(sessionId: string, severity: number): Promise<void> {
  ...
  private calculateMatchScore(volunteer: any, criteria: VolunteerMatchCriteria): number {
    let score = 0;
  ...
  private calculateSpecializationMatch(
    volunteerSpecs: string[],
    crisisKeywords: string[],
    requestedSpecs?: string[]
  ): number {
    // Crisis-specific specializations
    const crisisSpecMap: Record<string, string[]> = {
  ...
  private calculateAvailabilityScore(volunteer: any): number {
  ...
  private calculateLanguageMatch(
    ...
    const matches = requestedLanguages.filter(lang => 
      volunteerLanguages.includes(lang)
    ).length;
