...
} from './monitoring/escalation-monitor';
 ...
 */
export async function initializeCrisisSystem(): Promise<{
 ...
 */
async function validateCrisisSystem(
  escalationSystem: EmergencyEscalationEngine,
  monitor: EscalationMonitor
): Promise<{
  isValid: boolean;
 ...
 */
export async function shutdownCrisisSystem(): Promise<void> {
 ...
 */
export async function getCrisisSystemStatus(): Promise<{
  ...
  try {
    ...
    const criticalAlerts = activeAlerts.filter((a: {severity: string}) => a.severity === 'CRITICAL');
    
    let status: 'OPERATIONAL' | 'DEGRADED' | 'CRITICAL' | 'OFFLINE' = 'OPERATIONAL';
